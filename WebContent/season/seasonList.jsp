...
<%@ page import="cs4111.form.seasonForm" %>
...
<html:form method="post" action="/season.do?method=querySeason">
    <html:hidden property="season_abstract"/>
    <input type="hidden" id="real_path" value="<%=realpath%>"/>

<table id="search" width="100%" align="center" cellpadding="0" cellspacing="0">
    <tr>
        <td align="center"><h2 class="myh2">Season</h2></td>
    </tr>
</table>
 ...
 <table id="search_table" width="100%" align="center" class="font12blue mytablegray  tdbgcolor" cellpadding="0" cellspacing="0">
        <tr class="font12blue tdbgcolor">
        	<td colspan="3">This Page includes the information for season!</td>
        </tr>
    ...
    <tr id="tablehead">
        ...
        <td align="center" bgcolor="#cccccc">Abstract</td>
        <td align="center" bgcolor="#cccccc">Aired from</td>
        <td align="center" bgcolor="#cccccc">Aired until</td>
        <td align="center" bgcolor="#cccccc">Characters</td>
    </tr>
    <%
    if(queryResult!=null)
    {
        for (int i = 0; i < queryResult.size(); i++) {
            ...
            seasonForm tempForm = (seasonForm) queryResult.get(i);
    %>
      <tr>
        <td align="center"><%=tempForm.getSeason_id()%>
        </td>
        <td align="center" width="50%"><%=tempForm.getSeason_abstract()%>
        </td>
        <td align="center"><%=tempForm.getAired_from()%>
        </td>
        <td align="center"><%=tempForm.getAired_until()%>
        </td>
        <td align="center" width="20%"><%=tempForm.getCharacter()%>
        </td>
      <tr>
    <%
        }
...
<script language="javascript">
    ...
    form.action = "<%=realpath%>/season.do?method=querySeason&currentPage=" + currentPage;
    form.submit();
	}
    ...
    function searchVideo()
    {
    	...
    	document.getElementsByName("season_abstract")[0].value=document.getElementsByName("fname")[0].value;
        document.forms[0].action = "<%=realpath%>/season.do?method=querySeason";
