...
<%@ page import="cs4111.form.castForm" %>
...
<html:form method="post" action="/cast.do?method=queryCast">
    <html:hidden property="cast_name"/>
    <input type="hidden" id="real_path" value="<%=realpath%>"/>

<table id="search" width="100%" align="center" cellpadding="0" cellspacing="0">
    <tr>
        <td align="center"><h2 class="myh2">Cast</h2></td>
    </tr>
</table>
 ...
 <table id="search_table" width="100%" align="center" class="font12blue mytablegray  tdbgcolor" cellpadding="0" cellspacing="0">
        <tr class="font12blue tdbgcolor">
        	<td colspan="3">This Page includes the information for cast!</td>
        </tr>
    ...
    if(queryResult!=null)
    {
        for (int i = 0; i < queryResult.size(); i++) {
            ...
            castForm tempForm = (castForm) queryResult.get(i);
    %>
      <tr>
...
<script language="javascript">
    ...
    form.action = "<%=realpath%>/cast.do?method=queryCast&currentPage=" + currentPage;
    form.submit();
	}
    ...
    function searchVideo()
    {
    	...
    	document.getElementsByName("cast_name")[0].value=document.getElementsByName("fname")[0].value;
        document.forms[0].action = "<%=realpath%>/cast.do?method=queryCast";
    ...
    function deleteVideo(name)
    {
        ...
        ymPrompt.confirmInfo({message:'wanna delete?',handler:function(xx)
        {
            if (xx == "ok")
            {
            	document.getElementsByName("cast_name")[0].value=name;
                alert(document.getElementsByName("cast_name")[0].value);
                document.forms[0].action = "<%=realpath%>/cast.do?method=doDelete";
                document.forms[0].submit();
            }
            else
            {
